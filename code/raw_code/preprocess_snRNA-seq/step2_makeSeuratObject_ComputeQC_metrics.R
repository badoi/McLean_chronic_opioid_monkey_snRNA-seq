...
STARsoloDIR= 'data/raw_data/STARsolo_out2'
...
names(STARsolo_filtered_fn) = ss(STARsolo_filtered_fn,"(STARsolo_out2/)|(.Solo.out)",2) 
head(STARsolo_filtered_fn)

names(STARsolo_raw_fn) = ss(STARsolo_raw_fn,"(STARsolo_out2/)|(.Solo.out)",2)
...
for(sample in names(STARsolo_raw_fn)){
  ...
  if(dir.exists(de_soup_counts_dn)){
    ...
    colnames(out) = ss(colnames(out), '_', 3)
    write10xCounts(de_soup_counts_dn, out, version = '3', overwrite = TRUE)
  }
}
...
objList = lapply(objList, function(obj){
  ...
  obj[["percent.ribomt"]] <- PercentageFeatureSet(object = obj, pattern = "^MT-|^RP[SL]")
  ## estimate cells w/ high score likely "compromised"/low QC cells,
  obj <- RunMiQC(obj, percent.mt = "percent.ribomt", nFeature_RNA = "nFeature_RNA", 
...
objList = lapply(objList, SCTransform, method = "glmGamPoi", 
                 vars.to.regress = "percent.ribomt",verbose = TRUE)
